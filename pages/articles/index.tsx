...
export default function PostsPage({
  ...
  // Search function for dropdown results
  ...
  const applySearchToMainContent = useCallback(async (query: string) => {
    const trimmedQuery = query.trim();
  ...
  }, [resetToInitialPosts, cancelPreviousRequest]);
  ...
  }, [currentTag, cancelPreviousRequest]);
  ...
  const handleResultSelect = useCallback((post: Post) => {
    router.push(`/articles/${post.slug}`);
    setShowSearchResults(false);
    setSearchQuery("");
  }, [router]);

  // Handle search submission (Enter key or "View all results")
  ...
  useEffect(() => {
    ...
    // Don't search if query is the same as last processed query
    if (trimmedQuery === lastSearchQueryRef.current) {
      return;
    }
    ...
    // Simple debounce - 300ms delay for smooth typing
      ...
      <SEO
        ...
        canonical={
          currentTag
            ...
            : searchQuery
            ? `https://webofen.com/articles?search=${encodeURIComponent(searchQuery)}`
          ...
          <div className="md:w-[1250px] mx-auto">
            ...
            <nav className="mb-6 px-4" aria-label="breadcrumb">
              <ol className="flex items-center space-x-2 space-x-reverse text-sm text-gray-500">
                ...
                <li className="flex items-center">
                  <svg className="w-4 h-4 mx-1 transform rotate-180" fill="currentColor" viewBox="0 0 20 20">
            ...
            <div className="mb-8 md:p-0 p-4">
              <div className="flex flex-col md:flex-row gap-4 items-start md:items-center justify-between">
                ...
                {/* Search Bar */}
                <div className="md:pr-2 w-full md:w-1/3 relative z-50">
                  ...
                  {showSearchResults && searchResults.length > 0 && (
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden z-50 max-h-96 overflow-y-auto">
                      ...
                      {searchResults.map((post) => (
                        <div
                          key={post.id}
                          onClick={() => handleResultSelect(post)}
                      ...
                      <div className="p-3 border-t border-gray-100 bg-gray-50">
                        <button
                          ...
                          className="w-full text-center text-sm text-[#ff5084] hover:text-[#e04475] font-medium"
                        >
                          مشاهده همه نتایج
                        </button>
                      </div>
                    ...
                    <div className="absolute top-full left-0 right-0 mt-1 bg-white rounded-xl shadow-lg border border-gray-200 overflow-hidden z-50">
                      <div className="p-4 text-center">
                        ...
                        <button
                          ...
                          className="mt-2 text-xs text-[#ff5084] hover:text-[#e04475]"
                        >
                          حذف جستجو
                        </button>
                      </div>
              ...
              {activeFilters && (
                <div className="mt-4 flex flex-wrap items-center gap-2 bg-blue-50 p-3 rounded-lg">
                  ...
                  <span className="text-gray-500 text-sm mr-2">({posts.length} مقاله یافت شد)</span>
                </div>
              )}
            </div>
            ...
            {activeFilters && posts.length === 0 && !searchLoading && (
              <div className="text-center py-12 bg-white rounded-2xl shadow">
                <div className="text-6xl mb-4">🔍</div>
                                <p className="text-gray-500 text-lg mb-2">مقاله‌ای با فیلترهای انتخاب شده یافت نشد.</p>
                {currentTag && (
                  <p className="text-gray-400 text-sm mb-1">
                    تگ: <span className="text-[#ff5084] font-medium">{currentTag}</span>
                  </p>
                )}
                {searchQuery && (
                  <p className="text-gray-400 text-sm mb-4">
                    جستجو: <span className="text-[#ff5084] font-medium">{searchQuery}</span>
                  </p>
                )}
                <button
                  onClick={() => {
                ...
                <div className="md:flex justify-between">
                      ...
                      >
                        <div className="rounded-2xl bg-white mb-8 shadow hover:shadow-lg transition cursor-pointer">
                          ...
                          <div className="w-full mt-6 p-6">
                            <div className="flex items-center justify-between">
                              <div className="flex items-center">
                                {postsWithViews[0].views > 10 && (
                                  <div className="relative group inline-block">
                                    <svg className="w-5 h-5 ml-2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                      <path d="M2 9.1371C2 14 6.01943 16.5914 8.96173 18.9109C10 19.7294 11 20.5 12 20.5C13 20.5 14 19.7294 15.0383 18.9109C17.9806 16.5914 22 14 22 9.1371C22 4.27416 16.4998 0.825464 12 5.50063C7.50016 0.825464 2 4.27416 2 9.1371Z" fill="#ff5084"/>
                                    </svg>
                                ...
                                {isRecommended(posts[0]) && (
                                  <div className="relative group inline-block">
                                    <svg className="w-5 h-5 ml-2" viewBox="0 0 36 36" xmlns="http://www.w3.org/2000/svg" aria-hidden="true" role="img" preserveAspectRatio="xMidYMid meet" fill="#000000">
                                ...
                                <p className="text-sm text-gray-500">{posts[0].category?.title}</p>
                              </div>
                              <span className="text-gray-400 text-sm">
                                {posts[0].createdAt
                                  ? new Date(posts[0].createdAt).toLocaleDateString("fa-IR", {
                                      year: "numeric",
                                      month: "long",
                                      day: "numeric",
                                    })
                            ...
                            <p className="text-gray-700 mt-1 text-xl font-semibold">{posts[0].title}</p>
                            <p className="text-gray-500 mt-3 text-md">{posts[0].description}</p>
                            ...
                            <div className="text-xs text-gray-400 mt-2 flex justify-between items-center">
                              <div className="flex items-center">
                                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                  <path d="M5.926 20.574a7.26 7.26 0 0 0 3.039 1.511c.107.035.179-.105.107-.175-2.395-2.285-1.079-4.758-.107-5.873.693-.796 1.68-2.107 1.608-3.865 0-.176.18-.317.322-.211 1.359.703 2.288 2.25 2.538 3.515.394-.386.537-.984.537-1.511 0-.176.214-.317.393-.176 1.287 1.16 3.503 5.097-.072 8.19-.071.071 0 .212.072.177a8.761 8.761 0 0 0 3.003-1.442c5.827-4.5 2.037-12.48-.43-15.116-.321-.317-.893-.106-.893.351-.036.95-.322 2.004-1.072 2.707-.572-2.39-2.478-5.105-5.195-6.441-.357-.176-.786.105-.75.492.07 3.27-2.063 5.352-3.922 8.059-1.645 2.425-2.717 6.89.822 9.808z" fill="#e16f23"/>
                                </svg>
                                <span className="mt-2 mr-1 text-sm">{posts[0].countview}</span>
                              </div>
                              <p className="text-gray-400 text-sm">زمان مطالعه : {posts[0].readtime} دقیقه</p>
                            </div>
                          </div>
                        </div>
                      </a>
                  ...
                  <div className="md:w-[35%] md:mr-4 md:p-0 p-4">
                    <div className="flex flex-col items-center justify-between h-full pb-3">
                      <a href="https://t.me/webofenlearn/4" target="_blank" rel="noopener noreferrer" className="relative bg-[#fbb2bd] mb-5 w-full aspect-[16/8] rounded-2xl border-3 border-white overflow-hidden">
                        <Image src="/blog/programming-learn (1).png" alt="آموزش برنامه نویسی" fill className="object-cover" priority/>
                      ...
                      <a href="https://t.me/webofenlearn/6" target="_blank" rel="noopener noreferrer" className="relative bg-[#fbb2bd] mb-5 w-full aspect-[16/8] rounded-2xl border-3 border-white overflow-hidden">
                        <Image src="/blog/ui-ux.png" alt="آموزش ui ux" fill className="object-cover" priority/>
                        <div className="absolute text-white top-[35%] right-5">
                          <p className="text-lg">آموزش</p>
                          <p className="text-2xl font-semibold">طراحی UI / UX</p>
                        </div>
                      </a>
                      <a href="https://t.me/webofenlearn/2" target="_blank" rel="noopener noreferrer" className="relative bg-[#fbb2bd] mb-5 w-full aspect-[16/8] rounded-2xl border-3 border-white overflow-hidden">
                        <Image src="/blog/seo.png" alt="آموزش سئو سایت" fill className="object-cover" priority/>
                        <div className="absolute text-white top-[35%] right-5">
                          <p className="text-lg">آموزش</p>
                ...
                {posts.length > 1 && (
                  ...
                  >
                    {cards.map((card, index) => (
                      <div key={"id" in card ? card.id : `post-${index}`} className="mb-8 break-inside-avoid">
                        {"type" in card && card.type === "social" ? (
                          <div className="rounded-2xl bg-white text-white shadow hover:shadow-sm transition cursor-pointer block">
                            <div className="rounded-2xl bg-white shadow hover:shadow-sm transition cursor-pointer block w-full">
                              {card.id === "telegram" && (
                                <a href="https://t.me/webofenlearn" target="_blank" rel="noopener noreferrer" className="block">
                              ...
                              {card.id === "instagram" && (
                                <a href="https://instagram.com/webofen" target="_blank" rel="noopener noreferrer" className="block">
                                  <div className="rounded-2xl bg-white text-white transition cursor-pointer block">
                                    <div className="flex items-center justify-between w-full">
                                      <div className="flex items-center">
                                        <div className="[background:linear-gradient(295deg,#f76290,#fe7464)] [box-shadow:-12px_0_hsla(4,99%,70%,.2),-22px_0_hsla(4,99%,70%,.1)] transform -skew-l-2 p-8 ml-10 rounded-2xl">
                                          <svg fill="#ffffff" height="30px" width="30px" viewBox="0 0 512 512" xmlns="http://www.w3.org/2000/svg">
                                            <g><path d="M256.551,100.198c-85.606,0-155.252,69.645-155.252,155.252c0,27.293,7.183,54.138,20.775,77.632 c2.284,3.947,7.336,5.295,11.284,3.014c3.947-2.284,5.297-7.335,3.014-11.284c-12.139-20.984-18.556-44.969-18.556-69.362 c0-76.499,62.236-138.735,138.735-138.735s138.735,62.236,138.735,138.735S333.050,394.185,256.551,394.185 c-38.491,0-74.271-15.397-100.748-43.355c-3.136-3.311-8.363-3.454-11.675-0.317c-3.311,3.136-3.454,8.363-0.317,11.675 c29.629,31.284,69.667,48.513,112.740,48.513c85.606,0,155.252-69.645,155.252-155.252S342.157,100.198,256.551,100.198z"/></g>
                                          </svg>
                                        </div>
                                        <div className="w-full">
                                          <p className="text-gray-500">
                                            <span className="block">در <span className="text-[#fb6c78] text-lg">اینستاگرام</span></span>
                                            <span>وبوفن را دنبال کنید!</span>
                                          </p>
                                        </div>
                                      </div>
                                      <div>
                                        <svg className="w-8 h-8 bg-red-50 rounded-full ml-8" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                          <path d="M14.2893 5.70708C13.8988 5.31655 13.2657 5.31655 12.8751 5.70708L7.98768 10.5993C7.20729 11.3805 7.2076 12.6463 7.98837 13.427L12.8787 18.3174C13.2693 18.7079 13.9024 18.7079 14.293 18.3174C14.6835 17.9269 14.6835 17.2937 14.293 16.9032L10.1073 12.7175C9.71678 12.327 9.71678 11.6939 10.1073 11.3033L14.2893 7.12129C14.6799 6.73077 14.6799 6.0976 14.2893 5.70708Z" fill="#fb6c78"/>
                                        </svg>
                                      </div>
                                    </div>
                          ...
                          <a href={`/articles/${(card as Post).slug}`} target="_blank" rel="noopener noreferrer">
                            <div className="rounded-2xl bg-white shadow hover:shadow-sm transition cursor-pointer">
                              ...
                              <div className="flex flex-col justify-between p-8">
                                  ...
                                  <div className="flex items-center">
                                    ...
                                    <div className="relative group inline-block">
                                      {isRecommended(card as Post) && (
                                        <>
                                          <svg className="w-5 h-5 ml-2 text-yellow-400 cursor-pointer" fill="#f06330" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                    ...
                                    <p className="text-sm text-gray-500">{(card as Post).category?.title}</p>
                                  </div>
                                  <span className="text-gray-400 text-sm">
                                    {(card as Post).createdAt
                                      ? new Date((card as Post).createdAt).toLocaleDateString("fa-IR", {
                                          year: "numeric",
                                          month: "long",
                                          day: "numeric",
                                        })
                                ...
                                <p className="text-base font-semibold text-gray-700 hover:text-[#1d546b]">{(card as Post).title}</p>
                                <p className="text-sm text-gray-700 hover:text-[#1d546b]">{(card as Post).description}</p>
                                ...
                                <p className="text-md text-gray-400 mt-2 line-clamp-2">{(card as Post).desc}</p>
                                <div className="text-xs text-gray-400 mt-2 flex justify-between items-center">
                                  <div className="flex items-center">
                                    <svg className="w-5 h-6" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                      <path d="M5.926 20.574a7.26 7.26 0 0 0 3.039 1.511c.107.035.179-.105.107-.175-2.395-2.285-1.079-4.758-.107-5.873.693-.796 1.68-2.107 1.608-3.865 0-.176.18-.317.322-.211 1.359.703 2.288 2.25 2.538 3.515.394-.386.537-.984.537-1.511 0-.176.214-.317.393-.176 1.287 1.16 3.503 5.097-.072 8.19-.071.071 0 .212.072.177a8.761 8.761 0 0 0 3.003-1.442c5.827-4.5 2.037-12.48-.43-15.116-.321-.317-.893-.106-.893.351-.036.95-.322 2.004-1.072 2.707-.572-2.39-2.478-5.105-5.195-6.441-.357-.176-.786.105-.75.492.07 3.27-2.063 5.352-3.922 8.059-1.645 2.425-2.717 6.89.822 9.808z" fill="#e16f23"/>
                                    </svg>
                                    <span className="mt-2 mr-1 text-sm">{(card as PostWithViews).views}</span>
                                  </div>
                                  <p className="text-gray-400 text-sm">زمان مطالعه: {(card as Post).readtime} دقیقه</p>
                                </div>
                              </div>
                            </div>
                          </a>
