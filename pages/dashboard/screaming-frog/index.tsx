...
export interface ScreamingfrogItem {
  id: string;
  slug: string;
  productTitle: string;
...
const ScreamingfrogPage: React.FC = () => {
  const { isLoggedIn } = useAuth();
  const { orders, loading, error } = useUserOrders();
  const [inputValuesMap, setInputValuesMap] = useState<Record<string, any[]>>(
    {}
  );
  // Map orders to Screamingfrog
  useEffect(() => {
    const mappedScreamingfrog: ScreamingfrogItem[] = orders.flatMap((order) =>
      order.items
        .filter((item) => item.variant?.product?.slug === "screaming-frog")
        .map((item) => {
    ...
    setScreamingfrogsState(mappedScreamingfrog);
  }, [orders, inputValuesMap]);

  const [completedOrderId, setCompletedOrderId] = useState<string | null>(null);

  const [ScreamingfrogsState, setScreamingfrogsState] = useState<ScreamingfrogItem[]>([]);
  ...
  const selectedItem = ScreamingfrogsState.find(
    ...
    const fetchAllValues = async () => {
      ...
      for (const order of orders) {
        for (const item of order.items) {
          if (item.variant?.product?.slug === "screaming-frog") {
  ...
  const handleClick = async (id: string) => {
    const item = ScreamingfrogsState.find((i) => i.id === id);
    if (!item) return;
  ...
  const selectedCompletedOrder = ScreamingfrogsState.find(
    (item) => item.id === completedOrderId
  );
  const handleSubmit = async () => {
    try {
      await submitValues();
      setScreamingfrogsState((prev) =>
        prev.map((item) =>
  ...
  const getProgress = (item: ScreamingfrogItem) =>
    statusProgressMap[item.adminStatus ?? "pending"] ?? 0;

  const isDelayed = (item: ScreamingfrogItem) => {
  ...
  useEffect(() => {
    const delayedItems = ScreamingfrogsState.filter(
  ...
  const inProgressItems = ScreamingfrogsState.filter(
    (item) => item.adminStatus === "in_progress"
  );
  const delayedItems = ScreamingfrogsState.filter(
    (item) => item.adminStatus === "out_of_time"
  );
  const canceledItems = ScreamingfrogsState.filter(
    (item) => item.adminStatus === "cancelled"
  );
  const normalItems = ScreamingfrogsState.filter(
    (item) =>
  ...
  const recentScreamingfrogs = ScreamingfrogsState.filter(
  ...
  const pendingScreamingfrogs = ScreamingfrogsState.find((item) => item.status === "0");
  const historyOrders = ScreamingfrogsState.filter(
    (item) =>
...
export default ScreamingfrogPage;
