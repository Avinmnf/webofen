...
const LinkingPage: React.FC = () => {
  ...
  // Map orders to LinkingItem
  useEffect(() => {
    const mappedLinking: LinkingItem[] = orders.flatMap((order) =>
      order.items
        .filter((item) => item.variant?.product?.slug === "internal-linking")
        .map((item) => {
    ...
    setLinkingsState(mappedLinking);
  }, [orders, inputValuesMap]);

  const [completedOrderId, setCompletedOrderId] = useState<string | null>(null);

  const [LinkingsState, setLinkingsState] = useState<LinkingItem[]>([]);
  ...
  const selectedItem = LinkingsState.find(
    ...
    const fetchAllValues = async () => {
      ...
      for (const order of orders) {
        for (const item of order.items) {
          if (item.variant?.product?.slug === "linking") {
  ...
  const handleClick = async (id: string) => {
    const item = LinkingsState.find((i) => i.id === id);
    if (!item) return;
  ...
  const selectedCompletedOrder = LinkingsState.find(
    (item) => item.id === completedOrderId
  );
  const handleSubmit = async () => {
    try {
      await submitValues();
      setLinkingsState((prev) =>
        prev.map((item) =>
  ...
  const getProgress = (item: LinkingItem) =>
    statusProgressMap[item.adminStatus ?? "pending"] ?? 0;

  const isDelayed = (item: LinkingItem) => {
  ...
  useEffect(() => {
    const delayedItems = LinkingsState.filter(
  ...
  const inProgressItems = LinkingsState.filter(
    (item) => item.adminStatus === "in_progress"
  );
  const delayedItems = LinkingsState.filter(
    (item) => item.adminStatus === "out_of_time"
  );
  const canceledItems = LinkingsState.filter(
    (item) => item.adminStatus === "cancelled"
  );
  const normalItems = LinkingsState.filter(
    (item) =>
  ...
  const recentLinkings = LinkingsState.filter(
  ...
  const pendingLinkings = LinkingsState.find((item) => item.status === "0");
  const historyOrders = LinkingsState.filter(
    (item) =>
