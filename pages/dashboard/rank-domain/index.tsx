...
const RankupPage: React.FC = () => {
  const { isLoggedIn } = useAuth();
  const { orders, loading, error } = useUserOrders();
  const [inputValuesMap, setInputValuesMap] = useState<Record<string, any[]>>(
    {}
  );
  // Map orders to Rankup
  useEffect(() => {
    const mappedRankup: RankupItem[] = orders.flatMap((order) =>
      order.items
        .filter((item) => item.variant?.product?.slug === "rankup")
        .map((item) => {
    ...
    setRankupsState(mappedRankup);
  }, [orders, inputValuesMap]);

  const [completedOrderId, setCompletedOrderId] = useState<string | null>(null);

  const [RankupsState, setRankupsState] = useState<RankupItem[]>([]);
  ...
  const selectedItem = RankupsState.find(
    ...
    const fetchAllValues = async () => {
      ...
      for (const order of orders) {
        for (const item of order.items) {
          if (item.variant?.product?.slug === "rankup") {
  ...
  const handleClick = async (id: string) => {
    const item = RankupsState.find((i) => i.id === id);
    if (!item) return;
  ...
  const selectedCompletedOrder = RankupsState.find(
    (item) => item.id === completedOrderId
  );
  const handleSubmit = async () => {
    try {
      await submitValues();
      setRankupsState((prev) =>
        prev.map((item) =>
  ...
  const getProgress = (item: RankupItem) =>
    statusProgressMap[item.adminStatus ?? "pending"] ?? 0;

  const isDelayed = (item: RankupItem) => {
  ...
  useEffect(() => {
    const delayedItems = RankupsState.filter(
  ...
  const inProgressItems = RankupsState.filter(
    (item) => item.adminStatus === "in_progress"
  );
  const delayedItems = RankupsState.filter(
    (item) => item.adminStatus === "out_of_time"
  );
  const canceledItems = RankupsState.filter(
    (item) => item.adminStatus === "cancelled"
  );
  const normalItems = RankupsState.filter(
    (item) =>
  ...
  const recentRankup = RankupsState.filter(
  ...
  const pendingRankup = RankupsState.find((item) => item.status === "0");
  const historyOrders = RankupsState.filter(
    (item) =>
