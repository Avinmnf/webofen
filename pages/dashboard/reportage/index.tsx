...
const ReportagePage: React.FC = () => {
  const { isLoggedIn } = useAuth();
  const { orders, loading, error } = useUserOrders();
  const [inputValuesMap, setInputValuesMap] = useState<Record<string, any[]>>(
    {}
  );
  // Map orders to Reportage
  useEffect(() => {
    const mappedReportage: ReportageItem[] = orders.flatMap((order) =>
      order.items
        .filter((item) => item.variant?.product?.slug === "reportage")
        .map((item) => {
    ...
    setReportagesState(mappedReportage);
  }, [orders, inputValuesMap]);

  const [completedOrderId, setCompletedOrderId] = useState<string | null>(null);

  const [ReportagesState, setReportagesState] = useState<ReportageItem[]>([]);
  ...
  const selectedItem = ReportagesState.find(
    ...
    const fetchAllValues = async () => {
      ...
      for (const order of orders) {
        for (const item of order.items) {
          if (item.variant?.product?.slug === "reportage") {
  ...
  const handleClick = async (id: string) => {
    const item = ReportagesState.find((i) => i.id === id);
    if (!item) return;
  ...
  const selectedCompletedOrder = ReportagesState.find(
    (item) => item.id === completedOrderId
  );
  const handleSubmit = async () => {
    try {
      await submitValues();
      setReportagesState((prev) =>
        prev.map((item) =>
  ...
  const getProgress = (item: ReportageItem) =>
    statusProgressMap[item.adminStatus ?? "pending"] ?? 0;

  const isDelayed = (item: ReportageItem) => {
  ...
  useEffect(() => {
    const delayedItems = ReportagesState.filter(
  ...
  const inProgressItems = ReportagesState.filter(
    (item) => item.adminStatus === "in_progress"
  );
  const delayedItems = ReportagesState.filter(
    (item) => item.adminStatus === "out_of_time"
  );
  const canceledItems = ReportagesState.filter(
    (item) => item.adminStatus === "cancelled"
  );
  const normalItems = ReportagesState.filter(
    (item) =>
  ...
  const recentReportages = ReportagesState.filter(
  ...
  const pendingReportages = ReportagesState.find((item) => item.status === "0");
  const historyOrders = ReportagesState.filter(
    (item) =>
