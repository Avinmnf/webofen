...
const ClusterPage: React.FC = () => {
  ...
  useEffect(() => {
    const mappedCluster: ClusterItem[] = orders.flatMap((order) =>
      order.items
        .filter((item) => item.variant?.product?.slug === "cluster")
        .map((item) => {
    ...
    setClustersState(mappedCluster);
  }, [orders, inputValuesMap]);

  const [completedOrderId, setCompletedOrderId] = useState<string | null>(null);

  const [ClustersState, setClustersState] = useState<ClusterItem[]>([]);
  ...
  const selectedItem = ClustersState.find(
    ...
    const fetchAllValues = async () => {
      ...
      for (const order of orders) {
        for (const item of order.items) {
          if (item.variant?.product?.slug === "cluster") {
  ...
  const handleClick = async (id: string) => {
    const item = ClustersState.find((i) => i.id === id);
    if (!item) return;
  ...
  const selectedCompletedOrder = ClustersState.find(
  ...
  const getProgress = (item: ClusterItem) =>
    statusProgressMap[item.adminStatus ?? "pending"] ?? 0;

  const isDelayed = (item: ClusterItem) => {
  ...
  useEffect(() => {
    const delayedItems = ClustersState.filter(
  ...
  const inProgressItems = ClustersState.filter(
    (item) => item.adminStatus === "in_progress"
  );
  const delayedItems = ClustersState.filter(
    (item) => item.adminStatus === "out_of_time"
  );
  const canceledItems = ClustersState.filter(
    (item) => item.adminStatus === "cancelled"
  );
  const normalItems = ClustersState.filter(
    (item) =>
  ...
  const recentCluster = ClustersState.filter(
  ...
  const pendingContents = ClustersState.find((item) => item.status === "0");
  const historyOrders = ClustersState.filter(
    (item) =>
