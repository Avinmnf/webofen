...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  if (!slugParam) return res.status(404).end();

  // حذف .xml و -sitemap از آخر
  ...
  try {
    if (slug === "articles") {
      ...
      urls.push(...categories.map((c: any) => ({ slug: `category/${c.slug}`, lastmod: formatDate(c.updatedAt) })));
    } else {
