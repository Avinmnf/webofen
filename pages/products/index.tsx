  ...
  return (
    <div className="p-6 max-w-[1440px] mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
        {products.map((product) => (
          ...
          >
            ...
            <div className="p-4">
              <h3 className="text-lg font-semibold text-gray-900 dark:text-white truncate">{product.title}</h3>
              {product.description && (
                <p className="mt-2 text-sm text-gray-600 dark:text-gray-400 line-clamp-3">{product.description}</p>
              )}
            </div>
          </Link>
        ))}
      ...
      <div className="mt-10 flex justify-center items-center gap-4">
        ...
        <span className="text-gray-600 dark:text-gray-300 text-sm">صفحه {page}</span>
        <button
      ...
      {loading && <p className="mt-6 text-center text-gray-500 dark:text-gray-400 animate-pulse">در حال بارگذاری...</p>}
    </div>
  );
}
